...
float scaleBackward_f(in float x, in float f_frac) {
...
float3 scaleBackward(in float3 x, in float f_frac) {
    return float3(
...
float3 scaleBackward_r(in float3 x) {
    float f_frac = scaleby - floor(scaleby);
...
float4 mainImage(VertData v_in) : TARGET
{
    ...
    if (inverse) {
        if (vertical) {
            ...
            colorSum += image.Load(int3(xi, 0, 0)).xyz;
        } else {
            int yi = int(xy.y);
